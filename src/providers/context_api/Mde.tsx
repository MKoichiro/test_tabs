...
export const Mde = ({ children }: { children: ReactNode }) => {
    ...
    const getCodemirror = () => {
        if (!mdeRef.current) {
            console.error('mdeRef.currentがfalsyです。[Providers.tsx, getCodemirrorElm()]');
            return;
        }
    ...
    const handleOutsideClick = (e: MouseEvent) => {
        if ((e.target as Element).closest('.mde-modal-contents')) {
            return;
        }
        handleModalClose();
    };
    ...
    const handleModalOpen = (todoId: string | undefined) => {
        ...
        if (typeof todoId !== 'string') {
            console.error('targetTodoIdxがundefinedです。 [Mde.tsx handleModalOpen()]');
            return;
        }
    ...
    const handleModalClose = () => {
        ...
        if (!maskRef.current) {
            console.error('maskRef.currentがfalsyです。[MdeProvider.tsx, handleModalClose()]');
            return;
        }
        if (!modalRef.current) {
            console.error('modalRef.currentがfalsyです。[MdeProvider.tsx, handleModalClose()]');
            return;
        }
    ...
    useEffect(() => {
        ...
        if (!maskRef.current) {
            console.error('maskRef.currentがfalsyです。[MdeProvider.tsx, useEffect()]');
            return;
        }
        if (!modalRef.current) {
            console.error('modalRef.currentがfalsyです。[MdeProvider.tsx, useEffect()]');
            return;
        }
        if (!mdeRef.current) {
            console.error('mdeRef.currentがfalsyです。[MdeProvider.tsx, useEffect()]');
            return;
        }
